...
You need add `failure-domain.beta.kubernetes.io/region=<region>` and
`failure-domain.beta.kubernetes.io/zone=<zone>` labels for all nodes.
...
> Note:
> - It is possible to use the [etcd-operator helm chart](https://github.com/kubernetes/charts/tree/master/stable/etcd-operator) to deploy it, however, we find some issues in this chart, such as [outdated cluster template](https://github.com/kubernetes/charts/issues/1795), so we choose to use the resources directly. 
...
We are investigating some ways to create services with `LoadBalancer` type because it's necessary for service discovery in federation, one of them is called [keepalived-cloud-provider](https://github.com/munnerz/keepalived-cloud-provider).
...
You need to add the CoreDNS server to the pod’s nameserver resolv.conf chain in all the federated clusters as this self hosted CoreDNS server is not discoverable publicly. This can be achieved by adding the below line to dnsmasq container’s arg in kube-dns deployment.
...
* We tried to deploy the `etcd-operator` and the `CoreDNS` in a different namespace than `default`, but this setup fails.
...
* CoreDNS server is not discoverable publicly. 

Obs: We are investigating these limitations and soon we expected to report a more descriptive issue and maybe to contribute to solving them. 
